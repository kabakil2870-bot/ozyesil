...
export function generateCartWhatsAppLink(
  ...
  if (customerInfo && customerInfo.fullName) {
    ...
    if (customerInfo.phone) customerDetails += `• Telefon: ${customerInfo.phone}\n`;
    customerDetails += `• Teslimat Tercihi: ${customerInfo.deliveryType === 'store_pickup' ? 'Mağazadan Teslim' : 'Adrese Kargo Teslimatı'}\n`;
